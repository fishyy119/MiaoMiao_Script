    ...
    try:
        with Image.open(image_path) as img:
            ...
            if width >= 2000 or height >= 3000:
                ...
                left_image_path = os.path.join(output_dir, f"{base_name}_left{ext}")
                right_image_path = os.path.join(output_dir, f"{base_name}_right{ext}")

                # 保存分割后的图片
                left_img.save(left_image_path, quality=100)  # 保存左半部分图片
                right_img.save(right_image_path, quality=100)  # 保存右半部分图片
